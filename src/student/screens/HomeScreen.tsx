...
type ScheduleItem = {
  ...
  status?: boolean;
  otp?: string;
  otp_created_at?: string;  // ISO timestamp when OTP was generated by faculty
...
const ClassScheduleCard = ({
  ...
  // - Class is completed (status = true) AND OTP is available AND attendance is NOT marked AND schedule is NOT violated
  const isScheduleViolated = violatedSchedules.has(item.id.toString());
  const canMarkAttendance = item.status && item.otp && !item.attendance_marked && !isScheduleViolated;
  ...
  return (
    <View style={styles.card}>
      ...
      <View style={styles.actionButtons}>
        ...
        {isScheduleViolated && !item.attendance_marked && (
        ...
        {/* Show Mark Attendance button when class is completed AND has OTP AND attendance is NOT marked AND no violation */}
        {canMarkAttendance && (
          <TouchableOpacity
...
const HomeScreen: React.FC<HomeScreenProps> = ({ user, setIsLoggedIn, setUser, navigation }) => {
  ...
  const fetchSchedule = async () => {
    try {
      ...
      const transformedTodaySchedule = data.today_schedule?.map((item: any) => ({
        ...
        otp: item.otp,
        otp_created_at: item.otp_created_at,  // Backend should provide OTP creation timestamp
  ...
  const handleMarkAttendance = (classEndTime: number, item: ScheduleItem) => {
    // Calculate OTP expiry based on when faculty generated it
    let otpExpiryTime: number;
    
    if (item.otp_created_at) {
      // Backend provides OTP creation timestamp
      // OTP expires 30 seconds after it was created by faculty
      const otpCreatedTimestamp = new Date(item.otp_created_at).getTime();
      otpExpiryTime = otpCreatedTimestamp + 30000; // 30 seconds from creation
      ...
      // Fallback: If backend doesn't provide creation time, use current time
      // This gives student 30 seconds from when they click (less secure)
  ...
  // Auto-refresh when screen comes into focus (e.g., returning from OTP screen)
  useFocusEffect(
    useCallback(() => {
      fetchSchedule();
      checkViolatedSchedules(); // Also check for violations
    }, [selectedDate]) // Re-fetch when screen focuses and selectedDate changes
  );
